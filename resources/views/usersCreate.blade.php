@if (session()->get('user_id')<>'')
    ...
    <center><form method="POST" @IF (isset($user['inform_id'])) action="{{ route('usersCreate')}}" @ENDIF>
            ...
            {{ csrf_field() }}
            <table border ="1">
                  Логин: <input size="25" type="text" name="login" @IF (isset($user['login'])) value = {{$user['login']}}@endif>
                    </br>
                    </br>

               Пароль: <input size="25"  type="password" name="password" @IF (isset($user['password'])) value = {{$user['password']}}@endif>
            </br>
            </br>
            ...
            ФИО: <input size="25" type="text" name="FIO" size="25" @IF (isset($oneUserInfCheck['FIO'])) value = "{{$oneUserInfCheck['FIO']}}"@endif >
            </br>
            {{--ФИО: <textarea  type="text" name="FIO" >@IF (isset($oneUserInfCheck['FIO'])) {{$oneUserInfCheck['FIO']}}@endif </textarea>--}}
             </br>
            Страна: <input size="25" type="text" name="country" @IF (isset($oneUserInfCheck['country'])) value = {{$oneUserInfCheck['country']}}@endif>
            </br>
            </br>

            Город: <input size="25" type="text" name="city" @IF (isset($oneUserInfCheck['city'])) value = {{$oneUserInfCheck['city']}}@endif>
            </br>
            </br>
